...
$c0aceaa5503517a6$var$snowflakes.start = ()=>{
    if (!$c0aceaa5503517a6$var$flakes.length) $c0aceaa5503517a6$var$flakes = new Array($c0aceaa5503517a6$var$snowflakeCount).fill(null).map($c0aceaa5503517a6$var$createFlake);
    $c0aceaa5503517a6$var$flakes.forEach((flake)=>document.body.appendChild(flake.element)
    );
    $c0aceaa5503517a6$var$enableAnimation = true;
    window.requestAnimationFrame($c0aceaa5503517a6$var$step);
};
$c0aceaa5503517a6$var$snowflakes.stop = ()=>{
    $c0aceaa5503517a6$var$enableAnimation = false;
    $c0aceaa5503517a6$var$flakes.forEach((flake)=>flake.element.remove()
    );
};
$c0aceaa5503517a6$var$snowflakes.toggle = ()=>{
    $c0aceaa5503517a6$var$enableAnimation = !$c0aceaa5503517a6$var$enableAnimation;
    if ($c0aceaa5503517a6$var$enableAnimation) $c0aceaa5503517a6$var$snowflakes.start();
    else $c0aceaa5503517a6$var$snowflakes.stop();
};
$c0aceaa5503517a6$var$snowflakes.updateCount = (newCount)=>{
    newCount = parseInt(newCount);
    if (!newCount || newCount < 0) return;
    $c0aceaa5503517a6$var$snowflakes.stop();
    $c0aceaa5503517a6$var$snowflakeCount = newCount;
    $c0aceaa5503517a6$var$flakes = new Array($c0aceaa5503517a6$var$snowflakeCount).fill(null).map($c0aceaa5503517a6$var$createFlake);
    clearTimeout($c0aceaa5503517a6$var$timer);
    $c0aceaa5503517a6$var$timer = setTimeout(()=>{
        $c0aceaa5503517a6$var$snowflakes.start();
    }, 10);
};
$c0aceaa5503517a6$var$snowflakes.updateSpeed = (newSpeed)=>{
    if (!newSpeed && newSpeed !== 0) return;
    newSpeed = parseFloat(newSpeed);
    if (newSpeed < 0 || newSpeed > 10) return;
    $c0aceaa5503517a6$var$speed = newSpeed;
};
function $c0aceaa5503517a6$var$step() {
    if (!$c0aceaa5503517a6$var$enableAnimation) return;
    if ($c0aceaa5503517a6$var$windFactor > $c0aceaa5503517a6$var$windCap) $c0aceaa5503517a6$var$windFactor -= 0.1;
    else if ($c0aceaa5503517a6$var$windFactor < -$c0aceaa5503517a6$var$windCap) $c0aceaa5503517a6$var$windFactor += 0.1;
    else if (Math.random() > 0.9) // common wind for all flakes. only change every 10th step randomly.
    $c0aceaa5503517a6$var$windFactor += Math.random() / 10 - 0.05;
    $c0aceaa5503517a6$var$flakes.forEach((flake)=>{
        // apply wind to each flake based on its velocity
        flake.x += $c0aceaa5503517a6$var$windFactor * flake.velocity * $c0aceaa5503517a6$var$speed;
        if (flake.x > 107) flake.x = -6; // when clipped right side, move to left side
        if (flake.x < -7) flake.x = 106; // and vice versa
        // fall down
        flake.y += flake.velocity * $c0aceaa5503517a6$var$speed;
...
var $c0aceaa5503517a6$export$2e2bcd8739ae039 = $c0aceaa5503517a6$var$snowflakes;
